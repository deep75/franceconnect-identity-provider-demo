...
var defaults = {
    ...
    // cf. config example here : http://stackoverflow.com/questions/21085624/how-to-create-a-normal-sails-model-without-being-in-the-models-folder
    connections: {
        def: {
            adapter: 'mongo',
    ...
    return [function (req, res, next) {
        ...
        function (req, res, next) {
            Q(req.parsedParams).then(function (params) {
                ...
                // profile and openid are allowed by default ->> do not check
                reqsco.splice(reqsco.indexOf('profile'));
                reqsco.splice(reqsco.indexOf('openid'));
                req.session.scopes = {};
                var promises = [];
                req.model.consent.findOne({
                                ...
                                return deferred.promise.then(function (obj) {
                                    ...
                                    if ((obj.auth.redirectUri || params.redirect_uri) && obj.auth.redirectUri != params.redirect_uri) {
                                        throw {
                                            type: 'error',
                                            error: 'invalid_grant',
                                            msg: 'Redirection URI does not match.'
                                        };
                                    }
                    ...
                    .then(function (obj) {
                        ...
                        if (params.grant_type != 'client_credentials' && obj.auth.client.key != client_key) {
                            throw {
                                type: 'error',
                                error: 'invalid_grant',
                                msg: 'The code was not issued for this client.'
                            };
                        }
